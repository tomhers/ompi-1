...
static const char *pmix4x_get_nspace(opal_jobid_t jobid);
static void pmix4x_register_jobid(opal_jobid_t jobid, const char *nspace);
...
static void pmix4x_query(opal_list_t *queries,
                         opal_pmix_info_cbfunc_t cbfunc, void *cbdata);
static void pmix4x_log(opal_list_t *info,
                       opal_pmix_op_cbfunc_t cbfunc, void *cbdata);

static int pmix4x_register_cleanup(char *path, bool directory, bool ignore, bool jobscope);

const opal_pmix_base_module_t opal_pmix_pmix4x_module = {
    .legacy_get = legacy_get,
    /* client APIs */
    .init = pmix4x_client_init,
    .finalize = pmix4x_client_finalize,
    .initialized = pmix4x_initialized,
    .abort = pmix4x_abort,
    .commit = pmix4x_commit,
    .fence = pmix4x_fence,
    .fence_nb = pmix4x_fencenb,
    .put = pmix4x_put,
    .get = pmix4x_get,
    .get_nb = pmix4x_getnb,
    .publish = pmix4x_publish,
    .publish_nb = pmix4x_publishnb,
    .lookup = pmix4x_lookup,
    .lookup_nb = pmix4x_lookupnb,
    .unpublish = pmix4x_unpublish,
    .unpublish_nb = pmix4x_unpublishnb,
    .spawn = pmix4x_spawn,
    .spawn_nb = pmix4x_spawnnb,
    .connect = pmix4x_connect,
    .connect_nb = pmix4x_connectnb,
    .disconnect = pmix4x_disconnect,
    .disconnect_nb = pmix4x_disconnectnb,
    .resolve_peers = pmix4x_resolve_peers,
    .resolve_nodes = pmix4x_resolve_nodes,
    .query = pmix4x_query,
    .log = pmix4x_log,
    ...
    .generate_regex = pmix4x_server_gen_regex,
    .generate_ppn = pmix4x_server_gen_ppn,
    .server_register_nspace = pmix4x_server_register_nspace,
    .server_deregister_nspace = pmix4x_server_deregister_nspace,
    .server_register_client = pmix4x_server_register_client,
    .server_deregister_client = pmix4x_server_deregister_client,
    .server_setup_fork = pmix4x_server_setup_fork,
    .server_dmodex_request = pmix4x_server_dmodex,
    .server_notify_event = pmix4x_server_notify_event,
    .server_iof_push = pmix4x_server_iof_push,
    .server_setup_application = pmix4x_server_setup_application,
    .server_setup_local_support = pmix4x_server_setup_local_support,
    /* tool APIs */
    .tool_init = pmix4x_tool_init,
    .tool_finalize = pmix4x_tool_fini,
    /* utility APIs */
    .get_version = PMIx_Get_version,
    .register_evhandler = register_handler,
    .deregister_evhandler = deregister_handler,
    .notify_event = notify_event,
    .store_local = pmix4x_store_local,
    .get_nspace = pmix4x_get_nspace,
    .register_jobid = pmix4x_register_jobid
};
...
static void pmix4x_register_jobid(opal_jobid_t jobid, const char *nspace)
{
    ...
    jptr = OBJ_NEW(opal_pmix4x_jobid_trkr_t);
    (void)strncpy(jptr->nspace, nspace, PMIX_MAX_NSLEN);
    jptr->jobid = jobid;
    opal_list_append(&mca_pmix_pmix4x_component.jobids, &jptr->super);
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
}
...
{
    pmix4x_threadshift_t *cd = (pmix4x_threadshift_t*)notification_cbdata;
    pmix4x_opcaddy_t *op;
    opal_value_t *kv;
    ...
    if (NULL != cd->pmixcbfunc) {
        ...
        if (NULL != results && 0 < (op->ninfo = opal_list_get_size(results))) {
            ...
            OPAL_LIST_FOREACH(kv, cd->info, opal_value_t) {
                ...
                pmix4x_value_load(&op->info[n].value, kv);
                ++n;
            }
        }
        /* convert the status */
        pstatus = pmix4x_convert_opalrc(status);
...
{
    pmix4x_threadshift_t *cd = (pmix4x_threadshift_t*)cbdata;
    opal_pmix4x_event_t *event;

    OPAL_PMIX_ACQUIRE_THREAD(&opal_pmix_base.lock);

    /* cycle thru the registrations */
    OPAL_LIST_FOREACH(event, &mca_pmix_pmix4x_component.events, opal_pmix4x_event_t) {
...
void pmix4x_event_hdlr(size_t evhdlr_registration_id,
                       ...
                       void *cbdata)
{
    ...
    if (NULL == source) {
        ...
        cd->pname.vpid = pmix4x_convert_rank(source->rank);
    }
    ...
    if (NULL != info) {
        ...
        for (n=0; n < ninfo; n++) {
            ...
            if (OPAL_SUCCESS != (rc = pmix4x_value_unload(iptr, &info[n].value))) {
                OPAL_ERROR_LOG(rc);
                OBJ_RELEASE(iptr);
                continue;
            }
    ...
    if (NULL != results) {
        for (n=0; n < nresults; n++) {
            ...
            if (OPAL_SUCCESS != (rc = pmix4x_value_unload(iptr, &results[n].value))) {
                OPAL_ERROR_LOG(rc);
                OBJ_RELEASE(iptr);
                continue;
            }
...
static int pmix4x_register_cleanup(char *path, bool directory, bool ignore, bool jobscope)
{
    ...
    if (jobscope) {
        ...
        /* only applies to us */
        rc = PMIx_Job_control_nb(&mca_pmix_pmix4x_component.myproc, 1, pinfo, ninfo, NULL, NULL);
    }
...
opal_vpid_t pmix4x_convert_rank(pmix_rank_t rank)
{
    switch(rank) {
...
pmix_rank_t pmix4x_convert_opalrank(opal_vpid_t vpid)
{
    switch(vpid) {
...
pmix_status_t pmix4x_convert_opalrc(int rc)
{
    switch (rc) {
...
int pmix4x_convert_rc(pmix_status_t rc)
{
    switch (rc) {
...
opal_pmix_scope_t pmix4x_convert_scope(pmix_scope_t scope)
{
    switch(scope) {
...
pmix_scope_t pmix4x_convert_opalscope(opal_pmix_scope_t scope) {
    switch(scope) {
...
pmix_data_range_t pmix4x_convert_opalrange(opal_pmix_data_range_t range) {
    switch(range) {
...
opal_pmix_data_range_t pmix4x_convert_range(pmix_data_range_t range) {
    switch(range) {
...
opal_pmix_persistence_t pmix4x_convert_persist(pmix_persistence_t persist)
{
    switch(persist) {
...
pmix_persistence_t pmix4x_convert_opalpersist(opal_pmix_persistence_t persist)
{
    switch(persist) {
...
char* pmix4x_convert_jobid(opal_jobid_t jobid)
{
    opal_pmix4x_jobid_trkr_t *jptr;
...
void pmix4x_value_load(pmix_value_t *v,
                       opal_value_t *kv)
{
    ...
    switch(kv->type) {
        ...
        case OPAL_STATUS:
            v->type = PMIX_STATUS;
            v->data.status = pmix4x_convert_opalrc(kv->data.status);
            break;
        case OPAL_VPID:
            v->type = PMIX_PROC_RANK;
            v->data.rank = pmix4x_convert_opalrank(kv->data.name.vpid);
            break;
        case OPAL_NAME:
            v->type = PMIX_PROC;
            ...
            found = false;
            OPAL_LIST_FOREACH(job, &mca_pmix_pmix4x_component.jobids, opal_pmix4x_jobid_trkr_t) {
            ...
            v->data.proc->rank = pmix4x_convert_opalrank(kv->data.name.vpid);
            break;
        ...
        case OPAL_PERSIST:
            v->type = PMIX_PERSIST;
            v->data.persist = pmix4x_convert_opalpersist((opal_pmix_persistence_t)kv->data.uint8);
            break;
        case OPAL_SCOPE:
            v->type = PMIX_SCOPE;
            v->data.scope = pmix4x_convert_opalscope((opal_pmix_scope_t)kv->data.uint8);
            break;
        case OPAL_DATA_RANGE:
            v->type = PMIX_DATA_RANGE;
            v->data.range = pmix4x_convert_opalrange((opal_pmix_data_range_t)kv->data.uint8);
            break;
        ...
        case OPAL_PTR:
            ...
            if (0 < v->data.darray->size) {
                ...
                OPAL_LIST_FOREACH(val, list, opal_value_t) {
                    ...
                    pmix4x_value_load(&info[n].value, val);
                    ++n;
                }
            } else {
        ...
        case OPAL_PROC_INFO:
            ...
            v->data.pinfo->proc.rank = pmix4x_convert_opalrank(kv->data.pinfo.name.vpid);
            ...
            v->data.pinfo->state = pmix4x_convert_opalstate(kv->data.pinfo.state);
            break;
        case OPAL_ENVAR:
            v->type = PMIX_ENVAR;
...
int pmix4x_value_unload(opal_value_t *kv,
                        const pmix_value_t *v)
{
    ...
    case PMIX_STATUS:
        kv->type = OPAL_STATUS;
        kv->data.status = pmix4x_convert_rc(v->data.status);
        break;
    case PMIX_VALUE:
    ...
    case PMIX_PROC:
        ...
        if (!found) {
            if (OPAL_SUCCESS != (rc = opal_convert_string_to_jobid(&kv->data.name.jobid, v->data.proc->nspace))) {
                return pmix4x_convert_opalrc(rc);
            }
        }
        kv->data.name.vpid = pmix4x_convert_rank(v->data.proc->rank);
        break;
    case PMIX_APP:
        OPAL_ERROR_LOG(OPAL_ERR_NOT_SUPPORTED);
        rc = OPAL_ERR_NOT_SUPPORTED;
    ...
    case PMIX_PERSIST:
        ...
        kv->data.uint8 = pmix4x_convert_persist(v->data.persist);
        break;
    case PMIX_POINTER:
        kv->type = OPAL_PTR;
    ...
    case PMIX_SCOPE:
        kv->type = OPAL_SCOPE;
        kv->data.uint8 = pmix4x_convert_scope(v->data.scope);
        break;
    case PMIX_DATA_RANGE:
        kv->type = OPAL_DATA_RANGE;
        kv->data.uint8 = pmix4x_convert_range(v->data.range);
        break;
    case PMIX_COMMAND:
        OPAL_ERROR_LOG(OPAL_ERR_NOT_SUPPORTED);
        rc = OPAL_ERR_NOT_SUPPORTED;
    ...
    case PMIX_PROC_INFO:
        ...
        kv->data.pinfo.name.vpid = pmix4x_convert_rank(v->data.pinfo->proc.rank);
        ...
        kv->data.pinfo.state = pmix4x_convert_state(v->data.pinfo->state);
        break;
    case PMIX_DATA_ARRAY:
        ...
        for (n=0; n < v->data.darray->size; n++) {
            ...
            if (PMIX_INFO == v->data.darray->type) {
                ...
                rc = pmix4x_value_unload(ival, &iptr[n].value);
                if (OPAL_SUCCESS != rc) {
    ...
    case PMIX_PROC_RANK:
        kv->type = OPAL_VPID;
        kv->data.name.vpid = pmix4x_convert_rank(v->data.rank);
        break;
    case PMIX_QUERY:
        OPAL_ERROR_LOG(OPAL_ERR_NOT_SUPPORTED);
        rc = OPAL_ERR_NOT_SUPPORTED;
                           ...
                           void *cbdata)
{
    ...
    if (NULL != op->evregcbfunc) {
        op->evregcbfunc(pmix4x_convert_rc(status), errhandler_ref, op->cbdata);
    }
    OBJ_RELEASE(op);
}
                             ...
                             void *cbdata)
{
    ...
    if (NULL != event_codes) {
        ...
        OPAL_LIST_FOREACH(kv, event_codes, opal_value_t) {
            op->pcodes[n] = pmix4x_convert_opalrc(kv->data.integer);
            ++n;
        }
    }
    ...
    /* register the event */
    op->event = OBJ_NEW(opal_pmix4x_event_t);
    op->event->handler = evhandler;
    opal_list_append(&mca_pmix_pmix4x_component.events, &op->event->super);
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);

    PMIx_Register_event_handler(op->pcodes, op->ncodes,
                                op->info, op->ninfo,
                                pmix4x_event_hdlr, errreg_cbfunc, op);
    return;
}
                        ...
                        opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == source) {
        ...
        if (NULL == (nsptr = pmix4x_convert_jobid(source->jobid))) {
            OBJ_RELEASE(op);
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_ERR_NOT_FOUND;
        }
        (void)strncpy(p.nspace, nsptr, PMIX_MAX_NSLEN);
        p.rank = pmix4x_convert_opalrank(source->vpid);
        pptr = &p;
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);

    /* convert the range */
    prange = pmix4x_convert_opalrange(range);
    ...
    if (NULL != info && 0 < (op->ninfo = opal_list_get_size(info))) {
        ...
        OPAL_LIST_FOREACH(kv, info, opal_value_t) {
            ...
            if (0 == strcmp(kv->key, OPAL_PMIX_JOB_TERM_STATUS)) {
                ...
                op->info[n].value.data.status = pmix4x_convert_opalrc(kv->data.integer);
            } else {
                pmix4x_value_load(&op->info[n].value, kv);
            }
            ++n;
    ...
    return pmix4x_convert_rc(pstatus);
}
...
static void infocbfunc(pmix_status_t status,
                       ...
                       void *release_cbdata)
{
    pmix4x_opcaddy_t *cd = (pmix4x_opcaddy_t*)cbdata;
    int rc = OPAL_SUCCESS;
    ...
    OPAL_LIST_FOREACH(q, queries, opal_pmix_query_t) {
        ...
        if (0 < cd->queries[n].nqual) {
            ...
            OPAL_LIST_FOREACH(ival, &q->qualifiers, opal_value_t) {
                ...
                pmix4x_value_load(&cd->queries[n].qualifiers[nq].value, ival);
                ++nq;
            }
        }
        ++n;
                       ...
                       opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    OPAL_LIST_FOREACH(ival, info, opal_value_t) {
        ...
        pmix4x_value_load(&cd->info[n].value, ival);
        ++n;
    }
...
opal_pmix_alloc_directive_t pmix4x_convert_allocdir(pmix_alloc_directive_t dir)
{
    switch (dir) {
        case PMIX_ALLOC_NEW:
...
int pmix4x_convert_state(pmix_proc_state_t state)
{
    switch(state) {
...
pmix_proc_state_t pmix4x_convert_opalstate(int state)
{
    switch(state) {
        case 0:
...
OBJ_CLASS_INSTANCE(opal_pmix4x_jobid_trkr_t,
                   opal_list_item_t,
                   NULL, NULL);

static void evcon(opal_pmix4x_event_t *p)
{
    OPAL_PMIX_CONSTRUCT_LOCK(&p->lock);
    p->handler = NULL;
    p->cbdata = NULL;
}
static void evdes(opal_pmix4x_event_t *p)
{
    OPAL_PMIX_DESTRUCT_LOCK(&p->lock);
}
OBJ_CLASS_INSTANCE(opal_pmix4x_event_t,
                   opal_list_item_t,
                   evcon, evdes);

static void opcon(pmix4x_opcaddy_t *p)
{
...
static void opdes(pmix4x_opcaddy_t *p)
{
    OPAL_PMIX_DESTRUCT_LOCK(&p->lock);
    if (NULL != p->nspace) {
        free(p->nspace);
...
OBJ_CLASS_INSTANCE(pmix4x_opcaddy_t,
                   opal_object_t,
                   opcon, opdes);

static void ocadcon(pmix4x_opalcaddy_t *p)
{
...
static void ocaddes(pmix4x_opalcaddy_t *p)
{
    OPAL_LIST_DESTRUCT(&p->procs);
    OPAL_LIST_DESTRUCT(&p->info);
    OPAL_LIST_DESTRUCT(&p->apps);
}
OBJ_CLASS_INSTANCE(pmix4x_opalcaddy_t,
                   opal_object_t,
                   ocadcon, ocaddes);

static void tscon(pmix4x_threadshift_t *p)
{
    OPAL_PMIX_CONSTRUCT_LOCK(&p->lock);
    p->msg = NULL;
    p->strings = NULL;
...
static void tsdes(pmix4x_threadshift_t *p)
{
    OPAL_PMIX_DESTRUCT_LOCK(&p->lock);
    if (NULL != p->strings) {
        free(p->strings);
...
OBJ_CLASS_INSTANCE(pmix4x_threadshift_t,
                   opal_object_t,
                   tscon, tsdes);

static void dmcon(opal_pmix4x_dmx_trkr_t *p)
{
    p->nspace = NULL;
    p->cbfunc = NULL;
    p->cbdata = NULL;
}
static void dmdes(opal_pmix4x_dmx_trkr_t *p)
{
    if (NULL != p->nspace) {
        free(p->nspace);
    }
}
OBJ_CLASS_INSTANCE(opal_pmix4x_dmx_trkr_t,
                   opal_list_item_t,
                   dmcon, dmdes);
